...
import { EPSG_DB, searchEPSG, type CRS } from '../reproject';
...
/** True when `key` is safe to use as a dynamic object key. */
...
type FieldName = 'lineName' | 'point' | 'rtype' | 'easting' | 'northing' | 'elevation' | 'lat' | 'lon' | 'idx';
 ...
 */
const SYNONYMS: Record<string, FieldName> = {
  // line
...
/**
 * Resolve a `# CRS:` / `# EPSG:` comment tag to an {@link SPSProjection}.
 * Accepts forms like `# CRS: ITM`, `# EPSG:2039`, `#crs=utm 36n`. Returns
 * undefined when the tag names nothing we recognise.
 */
function resolveCrsTag(tag: string): SPSProjection | undefined {
...
export function parseCoordCsv(text: string): SPSData {
  ...
  try {
    ...
    let projection: SPSProjection | undefined;
    for (const raw of lines) {
      const t = raw.trim();
      if (!t) continue;
      if (t.startsWith('#') || t.startsWith(';')) {
        if (/crs|epsg|coordinate|projection/i.test(t)) {
          const p = resolveCrsTag(t);
          if (p && !projection) projection = p;
        }
        continue;
      }
    ...
    for (let li = 0; li < lines.length; li++) {
      ...
      const easting = num(fields.easting);
      const northing = num(fields.northing);
    ...
    if (out.sources.length + out.receivers.length === 0 && out.skipped > 0) {
      out.errors.unshift('coordcsv: no points parsed — check the column layout (expected easting,northing[,elevation] or line,point,easting,northing,elevation)');
    }
  } catch (e) {
