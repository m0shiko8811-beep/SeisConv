 ...
 *
 * Every export also carries a matching `.prj` when the survey's CRS is known, so
 * the result can be opened in a GIS without the user hand-defining the projection.
 */
export function buildPositioningExport(kind: 'p111' | 'coordcsv' | 'segp1' | 'sps', data: SPSData): { name: string; text: string }[] {
  ...
  // anyway (coordinate CSV carries a `# CRS:` tag, P1/11 an H,CRS record).
  if (kind === 'sps') {
