...
function mkPoint(rtype: 'S' | 'R', lineName: string, point: number, pos: { e: number; n: number }, type?: string, elev = 0): SPSPoint {
  const p: SPSPoint = {
    rtype, lineName, point, idx: '1',
    easting: pos.e, northing: pos.n, elevation: isFinite(elev) ? elev : 0,
    raw: '', lineNum: 0,
  };
...
// -- PREPLOT generation --------------------------------------------------------
//
// The counterpart to generateSPS: the stations are already surveyed/designed, so
// they are placed VERBATIM and their point numbers are honoured. Nothing is
// re-sampled, nothing is interpolated.
//
// A separate entry point rather than a flag on CreateParams, because generateSPS's
// whole contract is "walk arc length at an interval": folding verbatim placement in
// would make rcvInterval, srcInterval and the four point-numbering parameters
// conditionally meaningless - six knobs that silently do nothing half the time.

/** One already-positioned station of a preplot line. */
export interface PreplotStation {
  e: number;
  n: number;
  /** The station number to write. Honoured exactly. */
  point: number;
  /** Elevation in metres; 0 when unknown. */
  elev?: number;
}

/** One preplot line: stations already positioned + numbered IN THE TARGET CRS. */
export interface PreplotLine {
  lineName: string;
  /** What these stations become. 'SR' emits a co-located source AND receiver per
   *  station, sharing the station number - the usual small-engineering-line case. */
  role: 'R' | 'S' | 'SR';
  stations: PreplotStation[];
}

export interface PreplotParams {
  lines: PreplotLine[];
  /** X model. 'full' = each shot records every receiver on its candidate lines;
   *  'split' = a window of `channels` receivers nearest the shot on each. */
  relation: { type: 'full' | 'split'; channels?: number };
  srcType?: string;
  rcvType?: string;
  /** First ffid to use, so a survey mixing generated and preplot lines keeps its
   *  ffids unique. Defaults to 1. */
  ffidStart?: number;
}

/** A receiver line indexed for cross-referencing. */
interface RcvIndex {
  name: string;
  points: number[];
  pos: { e: number; n: number }[];
}

/**
 * Generate an {@link SPSData} from PREPLOT lines.
 *
 * Every station is written at exactly the coordinate given, under exactly the point
 * number given - a client's pre-plot survives import → generate untouched, which is
 * the entire reason this path exists.
 *
 * Cross-references follow the same rule generateSPS's 2D path uses: a shot is
 * related to the receivers on ITS OWN line when that line carries receivers (the
 * co-located 2D case), and otherwise to every receiver line in the survey (a
 * source-line / receiver-line layout). `relation.type` then selects the full
 * template or a moving window of `channels` stations.
 *
 * Bounded by {@link MAX_GENERATED_POINTS} on both the station count and the X-row
 * count, checked BEFORE anything is allocated. Throws a stated message on invalid
 * input; the worker turns that into `{ok:false, error}`.
 */
export function generatePreplotSPS(params: PreplotParams, proj: SPSProjection): SPSData {
  if (!params || !Array.isArray(params.lines) || params.lines.length === 0)
    throw new Error('generatePreplotSPS: at least one preplot line is required');
  if (!params.relation || (params.relation.type !== 'full' && params.relation.type !== 'split'))
    throw new Error("generatePreplotSPS: relation.type must be 'full' or 'split'");

  const split = params.relation.type === 'split';
  const channels = split ? Math.floor(params.relation.channels ?? NaN) : 0;
  if (split && (!isFinite(channels) || channels <= 0))
    throw new Error(`generatePreplotSPS: relation.channels must be a finite number > 0 (got ${params.relation.channels})`);

  const ffidStart = params.ffidStart == null ? 1 : Math.floor(params.ffidStart);
  if (!isFinite(ffidStart) || ffidStart < 1)
    throw new Error(`generatePreplotSPS: ffidStart must be a finite number >= 1 (got ${params.ffidStart})`);

  // -- validate + bound BEFORE allocating (DoS discipline, as generateSPS does) --
  let totalPoints = 0;
  params.lines.forEach((ln, k) => {
    if (!ln || typeof ln.lineName !== 'string' || !ln.lineName.trim())
      throw new Error(`generatePreplotSPS: line ${k} has no name`);
    if (ln.role !== 'R' && ln.role !== 'S' && ln.role !== 'SR')
      throw new Error(`generatePreplotSPS: line ${ln.lineName} has an unsupported role '${ln.role}'`);
    if (!Array.isArray(ln.stations) || ln.stations.length === 0)
      throw new Error(`generatePreplotSPS: line ${ln.lineName} has no stations`);
    for (const s of ln.stations) {
      if (!s || !isFinite(s.e) || !isFinite(s.n))
        throw new Error(`generatePreplotSPS: line ${ln.lineName} has a non-finite station coordinate`);
      if (!isFinite(s.point))
        throw new Error(`generatePreplotSPS: line ${ln.lineName} has a non-finite station number`);
    }
    totalPoints += ln.stations.length * (ln.role === 'SR' ? 2 : 1);
  });
  if (totalPoints > MAX_GENERATED_POINTS)
    throw new Error(`generatePreplotSPS: ${totalPoints} points exceeds the cap of ${MAX_GENERATED_POINTS}`);

  // -- emit the stations verbatim --
  const sources: SPSPoint[] = [];
  const receivers: SPSPoint[] = [];
  const rcvIndex: RcvIndex[] = [];
  /** Shots in emission order, each remembering which line it sits on. */
  const shots: { lineName: string; point: number; pos: { e: number; n: number } }[] = [];

  for (const ln of params.lines) {
    const name = ln.lineName.trim();
    const wantR = ln.role === 'R' || ln.role === 'SR';
    const wantS = ln.role === 'S' || ln.role === 'SR';
    if (wantR) {
      const idx: RcvIndex = { name, points: [], pos: [] };
      for (const s of ln.stations) {
        const pos = { e: s.e, n: s.n };
        receivers.push(mkPoint('R', name, s.point, pos, params.rcvType, s.elev ?? 0));
        idx.points.push(s.point);
        idx.pos.push(pos);
      }
      rcvIndex.push(idx);
    }
    if (wantS) {
      for (const s of ln.stations) {
        const pos = { e: s.e, n: s.n };
        sources.push(mkPoint('S', name, s.point, pos, params.srcType, s.elev ?? 0));
        shots.push({ lineName: name, point: s.point, pos });
      }
    }
  }
  ...
  // -- cross-references --
  const xrefs: SPSXref[] = [];
  if (shots.length && rcvIndex.length) {
    // Bound the row count before building any of it.
    const maxCandidates = rcvIndex.length;
    if (shots.length * maxCandidates > MAX_GENERATED_POINTS)
      throw new Error(`generatePreplotSPS: ${shots.length * maxCandidates} X-relations exceeds the cap of ${MAX_GENERATED_POINTS} (use a smaller patch / fewer shots)`);

    const byName = new Map<string, RcvIndex>();
    for (const r of rcvIndex) if (!byName.has(r.name)) byName.set(r.name, r);

    let ffid = ffidStart;
    for (const shot of shots) {
      const own = byName.get(shot.lineName);
      const candidates = own ? [own] : rcvIndex;
      let ch0 = 0;
      for (const rl of candidates) {
        ...
        if (split) {
          ...
          let best = Infinity;
          for (let i = 0; i < nR; i++) {
            const d = dist2d(shot.pos, rl.pos[i]);
            if (d < best) { best = d; nearest = i; }
          }
          const half = Math.floor(channels / 2);
          winStart = Math.max(0, nearest - half);
          winEnd = Math.min(nR - 1, winStart + channels - 1);
          winStart = Math.max(0, winEnd - channels + 1); // re-clip if we hit the far end
        }
        const cnt = winEnd - winStart + 1;
        xrefs.push({
          tape: '', ffid, srcLine: shot.lineName, srcPt: shot.point, srcIdx: '1',
          fromCh: ch0 + 1, toCh: ch0 + cnt, chIncr: 1,
          rcvLine: rl.name, rcvPtFrom: rl.points[winStart], rcvPtTo: rl.points[winEnd], rcvIdx: '1',
          rcvLineFrom: rl.name, rcvLineTo: rl.name, layout: 'SPS2.1',
        });
        ch0 += cnt;
      }
      ffid++;
