...
export interface CRS {
  ...
  region?: string;
  /** Projection method for the non-Transverse-Mercator grids (LCC, MERC, AEA,
   *  LAEA, CASS, STERE, STEREA). Absent for the TM/UTM/GEO fast paths. */
  method?: string;
  /** Conic standard parallels / latitude of true scale. */
  lat1?: number;
  lat2?: number;
  latTs?: number;
  /** Prime-meridian offset from Greenwich, degrees (Ferro, Jakarta, Paris...). */
  pmOffset?: number;
  /** Metres per linear unit of the projected coordinates (feet grids). */
  unitFactor?: number;
  /** EPSG has retired this CRS; still usable, but flagged in the picker. */
  deprecated?: boolean;
}
 ...
 */
/**
 * The GEODETIC DATUM a CRS is actually on, for the H12 record.
 *
 * This used to be hardcoded to 'WGS 84 GRS 1980' for every TM/UTM grid, which
 * stamped that label onto the Israeli grid, the British National Grid and RD New -
 * none of which are on WGS 84. Worse, it contradicted the very next record: H14
 * carries the seven-parameter shift FROM the grid's own datum TO WGS 84, and a grid
 * that needed no shift would not have one. A receiving processor told a
 * Helmert-shifted grid is WGS 84 will place the survey wrong.
 */
function datumLabel(tgt: CRS): string {
  // An EPSG CRS name reads "<datum> / <projection>", so the part before the slash
  // IS the datum: "WGS 84 / UTM zone 36N" -> "WGS 84".
  const name = tgt.name || '';
  const slash = name.indexOf(' / ');
  if (slash > 0) return name.slice(0, slash).toUpperCase();
  // Names without that shape, spelled out so the record stays short enough for the
  // fixed 48-column field.
  if (tgt.code === 'EPSG:2039') return 'ISRAEL 1993';
  const hp = tgt.helmert;
  const shifted = !!hp && Math.abs(hp.dx) + Math.abs(hp.dy) + Math.abs(hp.dz) > 0.01;
  // No shift to WGS 84 is needed only because it already IS WGS 84.
  if (!shifted) return 'WGS 84';
  return (tgt.code || 'LOCAL DATUM').toUpperCase();
}

export function generateProjHeaders(tgt: CRS): Record<string, string> {
  const datumDesc = datumLabel(tgt);
...
/**
 * The non-Transverse-Mercator fields of a CRS, in the shape {@link Projection}
 * wants. Kept in one place so every CRS -> Projection conversion carries the
 * method, its standard parallels, the prime meridian and the linear unit; a
 * conversion that dropped them would silently fall back to Transverse Mercator
 * and project a Lambert grid as if it were a TM one.
 */
export function extraProjFields(crs: CRS): Partial<Projection> {
  return {
    method: crs.method,
    lat1: crs.lat1,
    lat2: crs.lat2,
    latTs: crs.latTs,
    pmOffset: crs.pmOffset,
    unitFactor: crs.unitFactor,
  };
}
...
export function reprojectSPS(srcLines: string[], tgt: CRS, spsType: 'S' | 'R' | 'X', sourceProj: SPSProjection): string {
  ...
  const tgtProj: Projection = { subtype: tgt.subtype, zone: tgt.zone, hemi: tgt.hemi, a: tgt.a, f: tgt.f, lon0: tgt.lon0, lat0: tgt.lat0, k0: tgt.k0, FE: tgt.FE, FN: tgt.FN, helmert: tgt.helmert, ...extraProjFields(tgt) };
