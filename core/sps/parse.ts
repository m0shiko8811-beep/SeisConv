 ...
 * phantom station (a real R 399/253 'KL' → phantom line "39", easting 253).
...
export function spsExtractProjection(headers: SPSHeader[]): SPSProjection {
  ...
  if (proj.subtype === 'UTM' || typeKey === 'UTM') {
    ...
    const h19 = get('H19');
    if (h19) {
      const zm = /(^\d{1,2})\b/.exec(h19.trim());
      if (zm) proj.zone = parseInt(zm[1]);
      proj.hemi = /south/i.test(h19) ? 'S' : 'N';
    }
  ...
  for (let li = 0; li < lines.length; li++) {
    ...
    if (t === 'S' || t === 'R') {
      ...
      if (t === 'S') {
        ...
        base.staticMs = sfloat(ln, 40, 46) || sfloat(ln, 44, 50);
