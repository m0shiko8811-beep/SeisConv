...
export function projToLatLon(E: number, N: number, proj?: Projection, elevM?: number): LatLon {
  ...
  const f = proj && proj.invF ? 1 / proj.invF : 1 / 298.257222101;
  const lon0 = (proj && proj.centralMeridian) || 0;
  const lat0 = (proj && proj.latOrigin) || 0;
  const k0 = (proj && proj.scaleFactor) || 1.0;
  const FE = proj && proj.falseEasting != null ? proj.falseEasting : 0;
  const FN = proj && proj.falseNorthing != null ? proj.falseNorthing : 0;
