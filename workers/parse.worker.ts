...
import { applyAGC, assistFirstBreaks, averageSpectrum, buildPositioningExport, buildRenumberMaps, buildSPS, checkGeometry, compareSPS, computeSemblance, CREATE_DEFAULTS, crsFromSpec, decodeSegyTrace, detect, detectPositioningFormat, detectSPSType, EPSG_DB, fkSpectrum, generateProjHeaders, generateSPS, getWriter, groupByLine, hRecCode, isIX1SegdTape, loadGeometry, lonLatToProj, MAX_SAMPLE_TRACES, MAX_SAMPLES_PER_TRACE, MAX_TRACES, mergeSPSData, nextPow2, normFactorPercentile, parseAny, parsePositioning, parseSEGY, parseSegyMeta, parseSPSText, projToLatLon, PROJ_HEADER_CODES, r16u, renumberSPSText, reprojectSPS, resampleLinear, runSPSQC, scanTraceHealth, EVIDENCE_STRIDE, writeEvidence, spectrogram, spsHeaderDesc, writeTapeRecord, type AGCType, type BinGrid, type CRS, type CreateParams, type CRSSpec, type DetectorId, type FBPolarity, type FBSeed, type GeomLoadResult, type HealthThresholds, type ParsedFile, type QCParams, type RenumberSpec, type SegyMeta, type Sensitivity, type SPSData, type SPSDeltaResult, type SPSHeader, type SPSPoint, type SPSProjection, type SurveyLine, type Trace, type TraceGeom } from '../core';
...
interface Req {
  ...
  kind?: 'kml' | 'geojson' | 'csv' | 'qcreport' | 'p111' | 'coordcsv';
  qcParams?: QCParams;
...
function buildSPSExport(d: SPSData, kind: 'kml' | 'geojson' | 'csv' | 'qcreport' | 'p111' | 'coordcsv', qcParams: QCParams): { ok: boolean; files: { name: string; text: string }[]; error?: string } {
  ...
  if (kind === 'p111' || kind === 'coordcsv') return { ok: true, files: buildPositioningExport(kind, d) };
...
port.on('message', (req: Req) => {
  ...
  try {
    ...
    if (type === 'spsReproject') {
      ...
      const tgt = EPSG_DB.find((c) => c.code === req.code);
      if (!tgt) {
    ...
    if (type === 'spsCreate') {
      ...
      if (picks.length > MAX_LINES) {
        port.postMessage({ id, ok: false, error: `spsCreate: ${picks.length} lines exceeds the cap of ${MAX_LINES}` });
        return;
      }
      let crs: CRS;
      ...
      const data = generateSPS(params, proj);
      ...
      currentBinGrid = null;
      currentBinGridSig = null;
      port.postMessage({ id, ok: true, summary: spsSummary(data, ['sps'], null), files: built });
      return;
    }
