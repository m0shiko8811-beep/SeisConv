...
### Added
- **Every positioning export now carries a matching `.prj`.** SPS 2.1, SEG-P1, IOGP P1/11 and coordinate CSV are all written with an ESRI WKT projection file named after the data file (`survey.s` / `survey.r` / `survey.x` -> `survey.prj`), so the exported points drop straight onto a map in QGIS, ArcGIS or Global Mapper already georeferenced. A positioning file states its CRS in its own header vocabulary, which no GIS reads; the `.prj` says the same thing in the one they all do. Generated surveys (SPS Creation -> Generate, and the direct SPS 2.1 export) get one too.
- A survey whose CRS is unknown, or whose projection cannot be described honestly in WKT, gets **no** `.prj` rather than a guessed one - an absent `.prj` reads as "CRS unknown" to every GIS, which is far safer than a plausible-looking projection that is not the one the coordinates are in.
