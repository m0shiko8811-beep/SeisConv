...
that runs only when you press it. The theme is the one the 0.8.1 entry set: a
claim the project makes about itself has to be checked against the code, and
where it turns out to be wrong it is corrected in the open rather than quietly
deleted.

### Fixed

- **The display clip level moved when reduced time was switched on.** The clip
  percentile in `renderer/src/app.ts` was measured on the SHIFTED matrix.
  Reduced time rewrites the panel and pads what moved out of the window with
  zeros, so those zeros joined the percentile and changed the exposure on a
  record nothing had been done to, which is precisely what the source says in as
  many words must not happen: reduced time is a pure shift in time and changes
  no sample value. The scale basis already obeyed that rule; the clip did not.
  Measured on a real record, the saturation value went from 0.0051 with reduced
  time off to 0.0199 with it on, about four times darker, with not one sample
  changed. The percentile is now measured on the un-shifted matrix, the same one
  the basis is already carried over from. With reduced time off that matrix IS
  the panel, so the ordinary display is untouched.
- **The pixel oracle could not see it.** Not one of the 196 states combined a
  clip below 100 with reduced time on: the clip sweep restores 100 before the
  reduced-time states run, so the hole sat exactly where this bug lived. A new
  state, `section:clip=90:reduce=on:v=8:mode=vd`, closes it, and was verified to
  fail when the fix is reverted. No existing key changes, and the oracle now
  stands at 197 states.

### Added

- **Check for updates, on demand only.** A Help panel action that asks GitHub
  for the latest release and reports what changed. It never checks on its own:
  no check at startup, no timer, no polling. It downloads and installs nothing,
  and hands the release page to your browser only if you ask it to. The answer
  is remembered, so a small badge stays until you dismiss it or until the
  running build catches up; remembering an answer contacts nothing. A release
  whose notes carry an explicit critical marker shows a stronger notice that
  names the reason, still dismissible and still installing nothing. The
  Microsoft Store build contains none of this code, because the Store handles
  updates itself. The tag comparison is pure and lives in `core/version.ts`,
  which takes the core suite from 484 tests to 492.
- **A privacy policy**, `PRIVACY.md`, written against the code rather than from
  a template.
- **Microsoft Store and winget packaging:** an `appx` target behind a separate
  `dist:store` script so the existing installer flow is untouched, the Store
  tile assets, and winget manifests under `packaging/winget/`.

### Changed

- **The README said WiFiSync verifies every file with a per-file SHA hash and
  that transfers are resumable. Neither was true.** There is no hashing anywhere
  in the WiFiSync code, and no resume: the temp file is always opened for
  writing from byte zero, and a partial is deleted on failure. What does protect
  a transfer is the peer's declared byte count and an atomic temp-then-rename,
  and both of those are real. The claim now describes what the code does.
  Corrected in the open rather than edited away, the same way the 0.8.0 AGC
  wording was.
- **"Two optional features touch the internet" was wrong twice.** It is four:
  the real-map basemap on the SPS tabs, the optional basemap band in a GeoTIFF
  export, Sync clock in the Observer's Log, and now Check for updates. None of
  the four is required, and none of them is contacted unless you ask for it. The
  in-app manual carried the same claim in a stronger form, naming the basemap as
  the only feature that uses the internet, and has been corrected with it.
- **Seven of the nine interface screenshots reshot, six of them from a real
  field record**, with the sections in the paper-section grey, replacing the
  synthetic set. The velocity semblance was reshot too but stays synthetic on
  purpose, because the real SEG-D carries no offsets and the app has to
  fabricate the trace spacing a semblance panel is drawn on. The two
  survey-geometry figures are unchanged: they remain the pair generated by
  `docs/manual/fixtures.mjs` at its artificial origin, because a map is
  coordinates by definition.
- **`npm run dist` now passes `--publish never`.** The pinned electron-builder
  publishes implicitly without it, which has been harmless only because no token
