...
<p align="center"><sub>The File Viewer. One shot record, 96 traces, 2 seconds, and the full display state printed inside the picture.</sub></p>
...
<p align="center"><sub>
  <b>Every picture on this page is synthetic.</b> They are captured by driving the built application over a
  deterministic, seeded SEG-Y set generated by <a href="docs/manual/fixtures.mjs"><code>docs/manual/fixtures.mjs</code></a>
  at an obviously artificial origin. No real survey, station, coordinate or client appears anywhere in this repository.
</sub></p>
...
### AGC will hide the fault you are looking for
...
One channel in this record was recorded at about a tenth of its neighbours. Under a time gain it is a pale
column nobody could miss. Switch AGC on, change nothing else, and it is gone. The per-trace attribute profile
is measured on the **stored** samples, so the notch survives every display setting there is. This is why the
gain-law picker says in plain words which laws equalise traces and which do not, and why the attribute profile
exists at all.
...
and the pixels, and states the file's declared SEG-Y polarity convention in the standard's own words rather
than as "normal" or "reverse", which mean opposite things in different parts of the world. It travels with the
image export, and whatever the panel was too narrow to show is repeated in full underneath.

### One record cannot show you a source going bad
...
side, one column per record, on a single common scale. Shot-to-shot behaviour only exists across records.
...
These are measurements taken during development on this machine, not guarantees. Your data and your hardware
...
| Installer | about **100 MB**; `app.asar` itself is about 12 MB, the rest is the Electron runtime |
