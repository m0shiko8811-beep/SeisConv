<p align="center">
  <img src="build/icon.png" alt="SeisConv" width="120"/>
</p>

<h1 align="center">SeisConv</h1>

<p align="center">
  <b>Open a SEG-Y and see it.</b><br/>
  A free, open source desktop viewer, converter and QC tool for field seismic data on Windows -
  SEG-Y, SEG-D, SEG-2 and Seismic Unix, with SPS survey geometry.<br/>
  Nothing to configure: format, revision, sample encoding and byte order are detected for you.
</p>
...
<p align="center">
  <img src="design/promo/hero-file-viewer.png" width="94%" alt="The SeisConv File Viewer showing a full shot record as a variable-density section"/>
</p>
<p align="center"><sub>The File Viewer. One shot record, 222 traces, the first two seconds of it, and the full display state printed inside the picture.</sub></p>

<p align="center">
  <b><a href="#download">Download for Windows</a></b> &nbsp;&middot;&nbsp;
  <a href="#the-argument-in-pictures">See it work</a> &nbsp;&middot;&nbsp;
  <a href="#format-support">Formats</a> &nbsp;&middot;&nbsp;
  <a href="#license">AGPL v3</a>
</p>

<p align="center"><sub>
  <b>The seismic in these figures is real field data, and it is anonymous.</b> The records were copied out of a
  field archive under neutral names, and nothing that identifies them travels with the samples: no survey, line,
  area, client or site name, no coordinate of any kind, no station, line or field-record number, no file name and
  no machine path. Every frame was checked against a forbidden-token denylist and read back by eye before it was
  written. The survey-geometry panels, the reduced-time figure and the velocity semblance are the exception: they are still generated by
  <a href="docs/manual/fixtures.mjs"><code>docs/manual/fixtures.mjs</code></a> at an obviously artificial origin,
  because geometry is coordinates by definition, and because these records carry no offset header, which both reduced time and a semblance panel need.
</sub></p>
...
It **runs offline** - no account, no cloud, no licence server - and nothing you open is uploaded anywhere. Four optional features touch the internet: the real-map basemap on the SPS tabs, which fetches map tiles; the optional basemap band in a GeoTIFF export, which downloads the tiles it resamples; Sync clock in the Observer's Log, which queries an NTP time server; and Check for updates in Help, which asks GitHub once whether a newer release has been published and downloads nothing. None of the four is required to use the app, none of them is contacted unless you ask for it, and everything else, the roughly 7,000-CRS EPSG registry included, works with no connection at all.
...
Twelve tabs cover the full office-and-field workflow: format conversion including batch to single tape-image combine (Converter), per-trace inspection with FFT QC (Trace Inspector), section visualization with first-break picking and trace-health QC (File Viewer), multi-format survey-geometry QC (SPS), survey design from a map (SPS Creation), NMO velocity analysis (Velocity), frequency and wavenumber analysis (Spectrum Analysis), multi-trace comparison and correlation (Trace Workbench), a configurable field-log grid (Observer's Log), SEG-Y to SPS cross-validation (Geometry QC), a vibroseis sweep designer and sweep QC (Sweeps), and built-in peer-to-peer file sharing over local WiFi with no router or configuration (WiFiSync). Multi-gigabyte files open through a streaming trace index, and any operation that can take more than a few seconds shows a progress bar.
...
- **No half-copied file, ever** - the sending peer declares each file's size up front; an implausible size is refused outright, and exactly that many bytes must arrive. The file streams into a temp file beside its destination and is renamed over it **atomically** only once it is complete, so a partial file never appears at the destination. A transfer that is cut short is deleted and pulled again on the next pass, not resumed, and no per-file hash is computed.
...
<sub>This figure is drawn on synthetic data, like the velocity semblance and for the same reason: reduced time needs the source-receiver offset
...
| What | Measured |
|---|---|
| Core unit tests | **492 passed, 0 failed, 4 skipped** without local sample data (`npm run test:core`) |
