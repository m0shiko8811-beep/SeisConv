...
- The **File Viewer** has ten display keys of its own - `F` fit, `+` / `-` zoom, `A` AGC, `M` mode, `C` colour map, `D` the Display panel, `R` reset, `PageUp` / `PageDown` paging. They are listed in full under **File Viewer** in the list on the left.
...
- **Exc** (excursion) - how far a sample at the clip level deflects, measured against the spacing to the *nearest neighbouring trace*. 1.0 is the usual default and just reaches that neighbour; above 1 the wiggles overlap their neighbours, so coherent events join into a continuous band. Under **trace spacing** by a geometry header the neighbour is wherever the header put it, so the wiggles stay separate where offsets bunch up and draw full size where the spread opens out. Two traces recorded at the same offset are drawn on top of each other, which is where they are.
...
- **Gain law** (in the **Display…** panel) - how the display brightens the data before the scale basis and the dB gain, applied after AGC. Six laws, with Seismic Unix `sugain` semantics: **None (true amplitude)** - raw sample values, the only mode you can quote an amplitude from. **Fixed gain** - one multiplier for everything; brightness changes, comparisons do not. **Time gain (t^n)** - brightens later arrivals to offset spreading loss, with the same correction on every trace, so a weak channel still looks weak. **Exponential time gain** - a stronger late-time boost, for heavily attenuated data. **Amplitude compression** - squashes the loud and quiet range so weak events show without flattening them; polarity and the order of amplitudes are kept. **Equalise traces (RMS)** - every trace ends at the same level; like AGC this *destroys* the amplitude evidence, so a weak channel can no longer be told from a healthy one by brightness, and it is not a mode for spread QC. The **n** box beside the picker is that law’s exponent, and each law remembers its own value.
...
- These work on the **File Viewer** only, with no modifier, and only while no dialog is open over it. Each key drives the toolbar control itself, so a key and a click do exactly the same thing and a greyed-out control does nothing either way. Every key is also written into its control’s own tooltip, so hovering a button tells you its key.
- `F` - fit the whole record · `+` / `-` - zoom the data in / out. Bare, with no Ctrl: `Ctrl +` and `Ctrl -` still zoom the whole interface instead.
- `A` - AGC on / off · `M` - next display mode (Variable Density, Wiggle, Variable Area, VD + Wiggle) · `C` - next colour map.
- `D` - show / hide the Display panel · `R` - reset the display, with the same Undo button you get from the Reset display button.
- `PageUp` / `PageDown` - previous / next block of traces, on a file large enough to be paged.
...
- AGC and **Equalise traces (RMS)** divide every trace by its own level, so a dying geophone does not go quiet under them: its own noise floor is what gets stretched toward full scale. What they destroy is the amplitude evidence, so brightness can no longer tell a weak channel from a healthy one, in either direction. For spread QC prefer **Time gain (t^n)**, whose correction depends only on time and is identical on every trace, or read the **Attributes** profile, which does not depend on the display at all.
