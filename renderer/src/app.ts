...
import { EPSG_DB, searchEPSG, type CRS } from '../../core/sps/reproject';
...
type WorkerProgress = { type: 'progress'; op: string; done: number; total: number; label: string };
...
declare global {
  interface Window {
    seisconvAPI: {
      ...
      spsExport(args: { kind: 'kml' | 'geojson' | 'csv' | 'qcreport' | 'p111' | 'coordcsv'; qcParams?: QCParamsUI }): Promise<{ ok: boolean; path?: string; canceled?: boolean; error?: string }>;
...
type HelpSection = { h: string; items: string[]; ordered?: boolean };
type HelpTopic = {
  title: string;
  what: string;
  controls: string[];
...
const MANUAL: Record<string, HelpTopic> = {
  general: {
    ...
    sections: [
      ...
      { h: 'Keyboard shortcuts', items: [
        ...
        '<span class="kbd" data-key="tabs">1–8</span> — switch tabs (1 = Converter … 9 = Observer Log)',
  ...
  trace: {
    ...
    controls: [
      ...
      '<b>Magnifier</b> (search icon) — drag a box over the trace to open that time/amplitude region enlarged in a zoom viewer (<span class="kbd">Esc</span> exits).',
  ...
  section: {
    ...
    controls: [
      ...
      '<b>Magnifier</b> — drag a box to open that trace/time region enlarged in a zoom viewer.',
      ...
      '<b>Hover read-out</b> — trace · time · amplitude under the cursor, with the trace’s FFID / CDP / node fetched as you hover.',
    ],
    sections: [
  ...
  sps: {
    ...
    controls: [
      ...
      '<b>Reproject</b> — pick a target EPSG and <b>Export reprojected (ZIP)…</b>.',
      '<b>Export</b> — KML, GeoJSON, CSV, IOGP P1/11, coordinates-CSV and the QC report.',
    ],
    sections: [
      ...
      { h: 'Creating / re-creating an SPS', items: [
        ...
        'Re-create or correct an existing one by loading it here, editing station fields in <b>Headers…</b>, reprojecting, and exporting fresh P1/11 or CSV.',
      ] },
    ],
    steps: [
    ...
    tips: [
      ...
      'No internet? Stay on <b>Survey grid</b>; only <b>Real map</b> needs to fetch basemap tiles.',
    ],
  },
  spscreate: {
    title: 'SPS Creation',
    what: 'Draw acquisition lines on a basemap, then generate a fresh SPS survey from them.',
    controls: [
      ...
      '<b>Undo pick</b> — remove the last vertex; <b>Clear</b> removes every line.',
    ...
    steps: [
      'Click the map to place the vertices of the first acquisition line.',
      'Use <b>+ Next line</b> for additional lines; <b>Undo pick</b> / <b>Clear</b> to correct.',
      'Review the auto-suggested <b>CRS</b> (override if needed).',
      'Click <b>Generate…</b>, confirm the parameters, then create &amp; save — the new survey opens on the <b>SPS</b> tab.',
    ],
    notes: [
  ...
  workbench: {
    ...
    controls: [
      ...
      '<b>A</b> / <b>B</b> selectors — choose two traces for cross-correlation, difference and stats (lag, peak correlation, RMS difference).',
      '<b>Clear all</b> — empty the collected list.',
      '<b>Export…</b> — save the collected set as a seismic file.',
    ],
    steps: [
      ...
      'Select <b>A</b> and <b>B</b> to read cross-correlation and difference.',
      'Click <b>Export…</b> to save the collected traces.',
    ],
    tips: [
  ...
  field: {
    ...
    controls: [
      ...
      '<b>WiFi hotspot</b> — <b>Name / Password</b> then <b>Start hotspot</b> to host a network from this PC (Windows Mobile Hotspot). <b>Status</b> reads the live state; <b>Host IP</b> shows the address to give the peer; <b>Open Windows Mobile Hotspot…</b> jumps to the Windows setting.',
...
// ── Send Feedback ───────────────────────────────────────────────────────────
// App version mirrored from package.json ("version"): no IPC channel exposes it,
// so it is hard-coded here (the documented fallback). The feedback inbox address
// lives ONLY in electron/main.ts (FEEDBACK_EMAIL) — the renderer just composes the
// subject + body and hands them to the main process to open via mailto:.
...
const TAB_META: Record<string, { title: string; desc: string }> = {
  ...
  spscreate: { title: 'SPS Creation', desc: 'Draw acquisition lines on a basemap, then generate a fresh SPS survey (sources · receivers · cross-references) from the picked geometry.' },
...
// LAST here on purpose. onKeyDown maps Ctrl/Cmd+1..9 to TABS[idx]; with more than
...
function switchTab(tab: Tab) {
  ...
  if (tab === 'spscreate') { ensureCreateMap(); createMap?.invalidateSize(); }
...
function activeTabHasData(tab: Tab): boolean {
  switch (tab) {
    ...
    case 'sps':
      return !!spsSummary;
    case 'spscreate':
      return createLines.some((l) => l.vertices.length > 0);
    case 'geomqc':
...
function clearActiveTab() {
  switch (activeTab) {
    ...
    case 'sps':
      ...
      break;
    case 'spscreate':
      audit('clear', `cleared SPS-creation picks (${createLines.reduce((a, l) => a + l.vertices.length, 0)} vertices)`, 'sps');
      createClear();
      break;
    case 'geomqc':
...
function clearSecHover() {
  ...
  setText('secHover', secBoxMode
    ...
    : 'Hover the section to read trace · time · amplitude.');
}
...
function hoverGrid(e: MouseEvent) {
  ...
  if (hit) {
    ...
    $('spsLabel').textContent = `Line ${g.names[g.line[i]]} · pt ${g.pt[i]} · ${g.x[i].toFixed(pr)}, ${g.y[i].toFixed(pr)}`;
  } else if (spsSummary) {
...
async function exportReprojected() {
  ...
  const code = ($('epsgSelect') as HTMLSelectElement).value;
...
function setSpsExportEnabled(on: boolean) {
  for (const id of ['spsExpKmlBtn', 'spsExpGeojsonBtn', 'spsExpCsvBtn', 'spsExpP111Btn', 'spsExpCoordCsvBtn', 'spsExpQcBtn']) {
...
async function exportSPS(kind: 'kml' | 'geojson' | 'csv' | 'qcreport' | 'p111' | 'coordcsv') {
  if (!spsSummary) {
    $('spsExpStatus').textContent = 'Load SPS files first.';
    return;
  }
  $('spsExpStatus').textContent = 'Exporting…';
  const kindLabel: Record<string, string> = { kml: 'KML', geojson: 'GeoJSON', csv: 'CSV', qcreport: 'QC report', p111: 'P1/11', coordcsv: 'coordinate CSV' };
  ...
  try {
    ...
    if (r.ok) { $('spsExpStatus').textContent = '✓ Saved ' + r.path; audit('export', `SPS ${kind} → ${r.path}`, 'sps'); }
    else if (r.canceled) $('spsExpStatus').textContent = '';
    else $('spsExpStatus').textContent = 'Failed: ' + (r.error || 'unknown');
  } catch (e) {
    $('spsExpStatus').textContent = 'Failed: ' + errMsg(e);
  } finally {
    hideProgress();
  }
}
...
/** One picked acquisition line: WGS84 vertices + its live Leaflet layers. */
type CreateLine = {
...
function ensureCreateMap() {
  ...
  createMap.on('click', (e: L.LeafletMouseEvent) => addCreateVertex(e.latlng.lat, e.latlng.lng));
...
/** The current line if it has ≥1 vertex (non-mutating — never opens a new line). */
function createActiveLine(): CreateLine | null {
  const l = createLines[createLines.length - 1];
  return l && l.vertices.length > 0 ? l : null;
}
...
/** Running length of a line's already-placed vertices (sum of segment metres). */
function createLineLength(line: CreateLine): number {
  let sum = 0;
  for (let i = 1; i < line.vertices.length; i++) {
    const d = createDistM(line.vertices[i - 1].lat, line.vertices[i - 1].lon, line.vertices[i].lat, line.vertices[i].lon);
    if (isFinite(d)) sum += d;
  }
  return sum;
}
...
function updateCreateCoords(lat: number, lon: number) {
  ...
  else if (createLines.every((l) => l.vertices.length === 0)) txt += '   ·   (pick a point to set the CRS)';
  // From-last distance + running length while a line is in progress (≥1 vertex).
  const active = createActiveLine();
  if (active) {
    const last = active.vertices[active.vertices.length - 1];
    ...
    if (active.vertices.length >= 2) {
      const lineLen = fmtCreateDist(createLineLength(active));
...
/** The line currently accepting vertices (creating the first one on demand). */
function createCurrentLine(): CreateLine {
  if (createLines.length === 0) startNewCreateLine();
  return createLines[createLines.length - 1];
}
function startNewCreateLine() {
  const color = CREATE_LINE_COLORS[createLines.length % CREATE_LINE_COLORS.length];
  createLines.push({ vertices: [], markers: [], polyline: null, color });
}
/** Rebuild a line's polyline layer from its current vertices (≥2 needed). */
function redrawCreatePolyline(line: CreateLine) {
  if (!createMap) return;
  if (line.polyline) { createMap.removeLayer(line.polyline); line.polyline = null; }
  if (line.vertices.length >= 2) {
    line.polyline = L.polyline(line.vertices.map((v) => [v.lat, v.lon] as [number, number]), { color: line.color, weight: 2, opacity: 0.85 }).addTo(createMap);
  }
}
/** Drop a vertex onto the current line (guards finite + geographic range). */
function addCreateVertex(lat: number, lon: number) {
  if (!isFinite(lat) || !isFinite(lon) || Math.abs(lat) > 90 || Math.abs(lon) > 180) return;
  ensureCreateMap();
  const map = createMap!;
  const firstEver = createLines.every((l) => l.vertices.length === 0);
  const line = createCurrentLine();
  line.vertices.push({ lat, lon });
  const m = L.circleMarker([lat, lon], { radius: 4, color: line.color, weight: 2, fillColor: line.color, fillOpacity: 0.9 }).addTo(map);
  line.markers.push(m);
  redrawCreatePolyline(line);
  // First vertex ever → suggest a CRS from the location (unless the user overrode it).
  if (firstEver && createCrsAuto) { createCrs = createSuggestCrs(lat, lon); updateCreateCrsBtn(); }
  renderCreatePicks();
  updateCreateCoords(lat, lon);
  setText('spsCreateLabel', `Line ${createLines.length} · ${line.vertices.length} vert${line.vertices.length === 1 ? 'ex' : 'ices'}`);
}
/** Finalise the current line (≥2 vertices) and start a fresh one. */
function createNextLine() {
  const cur = createLines[createLines.length - 1];
  if (!cur || cur.vertices.length < 2) { setText('spsCreateLabel', 'Finish the current line first (need ≥2 vertices).'); return; }
  startNewCreateLine();
  clearCreateRubberband();   // a finished line shouldn't trail a band into the next
  renderCreatePicks();
  setText('spsCreateLabel', `Started line ${createLines.length} — click to pick vertices`);
}
/** Remove the last vertex (and its marker); drop a line that becomes empty. */
function createUndo() {
  // Drop any trailing empty line(s) first (e.g. right after "Next line").
  while (createLines.length && createLines[createLines.length - 1].vertices.length === 0) {
    const empty = createLines.pop()!;
    if (empty.polyline && createMap) createMap.removeLayer(empty.polyline);
  }
  const cur = createLines[createLines.length - 1];
  if (!cur) { renderCreatePicks(); setText('spsCreateLabel', 'Nothing to undo'); return; }
  cur.vertices.pop();
  const m = cur.markers.pop();
  if (m && createMap) createMap.removeLayer(m);
  redrawCreatePolyline(cur);
  if (cur.vertices.length === 0) {
    if (cur.polyline && createMap) createMap.removeLayer(cur.polyline);
    createLines.pop();
  }
  clearCreateRubberband();   // band rebuilds from the new last vertex on next move
  renderCreatePicks();
  setText('spsCreateLabel', 'Removed last vertex');
}
/** Wipe every picked line + its layers (and reset the CRS to auto-suggest). */
function createClear() {
  if (createMap) {
    for (const l of createLines) {
      for (const m of l.markers) createMap.removeLayer(m);
      if (l.polyline) createMap.removeLayer(l.polyline);
    }
  }
  clearCreateRubberband();
  createLines = [];
...
/** Re-render the grouped picks list (one block per line; lat/long + E/N each). */
function renderCreatePicks() {
  const host = $opt('spsCreatePicks');
  const countEl = $opt('spsCreatePicksCount');
  if (!host) return;
  const lines = createLines.filter((l) => l.vertices.length > 0);
  const totalV = lines.reduce((a, l) => a + l.vertices.length, 0);
  if (countEl) countEl.textContent = `${lines.length} line${lines.length === 1 ? '' : 's'} · ${totalV} vert${totalV === 1 ? 'ex' : 'ices'}`;
  if (lines.length === 0) {
    host.innerHTML = '<div class="hdr-empty">No vertices picked yet — click the map to start a line.</div>';
    return;
  }
  host.innerHTML = '';
  lines.forEach((l, li) => {
    const grp = document.createElement('div');
    grp.className = 'hdr-grp';
    const h = document.createElement('h3');
    h.textContent = `Line ${li + 1} — ${l.vertices.length} vertices`;
    h.style.color = l.color;
    grp.appendChild(h);
    l.vertices.forEach((v, vi) => {
      const kv = document.createElement('div');
      kv.className = 'kv';
      const k = document.createElement('span'); k.className = 'k'; k.textContent = `#${vi + 1}`;
      const val = document.createElement('span'); val.className = 'v mono';
      const en = createProjectEN(v.lat, v.lon);
      val.textContent = `${v.lat.toFixed(6)}, ${v.lon.toFixed(6)}` + (en ? `   ·   E ${en.e.toFixed(1)} N ${en.n.toFixed(1)}` : '');
      kv.appendChild(k); kv.appendChild(val);
      grp.appendChild(kv);
    });
...
/** One-line wizard summary: mode · picked-line count (role) · vertices · CRS. */
function createWizardSummary(): string {
  const valid = createLines.filter((l) => l.vertices.length >= 2);
  const totalV = valid.reduce((a, l) => a + l.vertices.length, 0);
  const role = createMode === '3D' ? ' (receiver lines)' : '';
  return `${createMode} · ${valid.length} line${valid.length === 1 ? '' : 's'}${role} · ${totalV} vertices · CRS ${createCrsLabel(createCrs)}`;
}
/** Generate… → validate the picks, then open the wizard prefilled from defaults. */
function openCreateWizard() {
  const valid = createLines.filter((l) => l.vertices.length >= 2);
  const partial = createLines.some((l) => l.vertices.length === 1);
  if (valid.length === 0) { setText('spsCreateLabel', 'Pick at least one line with ≥2 vertices first.'); return; }
  if (partial) { setText('spsCreateLabel', 'A line has only one vertex — add another point or undo it.'); return; }
...
async function confirmCreateWizard() {
  const valid = createLines.filter((l) => l.vertices.length >= 2);
  if (valid.length === 0) { setText('spsWizardStatus', 'Pick at least one line with ≥2 vertices.'); return; }
  const numU = (id: string) => { const v = parseFloat(($(id) as HTMLInputElement).value); return isFinite(v) ? v : undefined; };
  const strU = (id: string) => { const v = ($(id) as HTMLInputElement).value.trim(); return v || undefined; };
  ...
  const req = {
    crs,
    baseName,
    picks: valid.map((l) => ({ vertices: l.vertices.map((v) => ({ lat: v.lat, lon: v.lon })) })),
    mode: createMode,
...
function initSpsCreate() {
  ...
  $opt('spsCreateUndo')?.addEventListener('click', createUndo);
  ...
  $opt('spsCreateCrsBtn')?.addEventListener('click', () => {
    ...
    if (createLines.some((l) => l.vertices.length >= 2)) openCreateWizard();
    else setText('spsCreateLabel', 'Pick a line first — the CRS is auto-suggested from your first vertex.');
  });
...
//  a UDP listener (localhost by default), and — next update — the trigger
...
function audit(action: string, detail: string, tab?: string) {
  ...
  if (!getSignature() && !sigPromptScheduled) {
    ...
    requestAnimationFrame(() => window.setTimeout(() => {
      ...
      if (!getSignature()) void ensureSignature();
    }, 500));
  }
...
type BackupKind = 'obslog' | 'workbench' | 'vel' | 'sps-draft';
...
function restoreLastBackup(kind: BackupKind): boolean {
  ...
  audit('restore', `restored "${snap.label}" snapshot`, kind === 'sps-draft' ? 'sps' : kind);
  return true;
}
...
/** Transient snackbar with an Undo button. onUndo runs at most once (double-undo
 *  guarded). Auto-dismisses after ~8s. */
...
function init() {
  ...
  workerProgOff = api.onWorkerProgress((p) => { updateProgress(p.done, p.total, p.label); });
  ...
  const epsgSel = $('epsgSelect') as HTMLSelectElement;
  for (const c of EPSG_DB) {
    const o = document.createElement('option');
    o.value = c.code;
    o.textContent = `${c.code} — ${c.name}`;
    epsgSel.appendChild(o);
  }
  epsgSel.value = 'EPSG:4326';
